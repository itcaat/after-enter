...
const russianStages: Stage[] = [
  {
    ...
    substeps: [
      ...
      { label: "Активное окно", detail: "Window manager направляет событие окну и элементу адресной строки браузера.", signal: "focused window → omnibox", ms: 1, exchanges: 0 },
  ...
  },
  {
    ...
    substeps: [
      ...
      { label: "DNS-запрос", detail: "Формируются запросы A, AAAA и при поддержке HTTPS-записей с уникальными ID.", signal: "QNAME · QTYPE · RD · EDNS", ms: 3, exchanges: 0 },
      ...
      { label: "Корневой сервер и TLD", detail: "Резолвер узнаёт у корневого сервера, кто отвечает за доменную зону.", signal: "root → .com nameserver", ms: 55, exchanges: 1 },
  ...
  },
  {
    ...
    substeps: [
      ...
      { label: "Server Hello", detail: "Сервер выбирает параметры и отправляет свою часть эфемерного ключа.", signal: "ServerHello · key_share", ms: 20, exchanges: 1 },
      { label: "Сертификат", detail: "Сервер отправляет сертификат со своим публичным ключом.", signal: "ServerHello + Certificate", ms: 45, exchanges: 1 },
      ...
      { label: "Finished", detail: "Обе стороны подтверждают целостность рукопожатия и переходят к передаче прикладных данных.", signal: "Finished ↔ Finished", ms: 15, exchanges: 1 },
  ...
  },
  {
    ...
    substeps: [
      ...
      { label: "TLS termination", detail: "CDN или балансировщик может завершить TLS и открыть новое защищённое соединение к origin-серверу.", signal: "client TLS → edge → origin TLS", ms: 8, exchanges: 1 },
  ...
  },
  {
    ...
    substeps: [
      { label: "Приём сетевой картой", detail: "Сетевая карта сервера проверяет кадр и помещает данные в receive ring.", signal: "wire → NIC RX ring", ms: 2, exchanges: 0 },
      { label: "DMA и interrupt", detail: "DMA переносит данные в память, а interrupt или polling пробуждает сетевой стек.", signal: "DMA → RAM · IRQ / NAPI", ms: 2, exchanges: 0 },
      ...
      { label: "Accept queue", detail: "Новое TCP-соединение попадает из SYN backlog в accept-очередь приложения.", signal: "SYN queue → accept queue", ms: 3, exchanges: 0 },
  ...
  },
  {
    ...
    substeps: [
      ...
      { label: "База данных", detail: "При промахе кеша приложение выполняет запрос и ждёт построения плана выполнения и I/O.", signal: "SQL / index → rows", ms: 35, exchanges: 1 },
  ...
  },
  {
    ...
    substeps: [
      ...
      { label: "Сторонние origin", detail: "Для стороннего домена могут повториться DNS, соединение, TLS и CORS-проверка.", signal: "new origin → DNS + connect + CORS", ms: 40, exchanges: 2 },
    ],
  },
  {
    key: "runtime", short: "JS", title: "Среда выполнения страницы", actor: "JavaScript engine",
    substeps: [
  ...
  {
    key: "render", short: "PIX", title: "Рендеринг страницы", actor: "Рендерер",
    substeps: [
  ...
  },
  {
    ...
    substeps: [
      ...
      { label: "Событие load", detail: "После загрузки критических зависимых ресурсов браузер переводит документ в complete.", signal: "readyState → complete · load", ms: 8, exchanges: 0 },
      { label: "Ленивая загрузка и фоновые ресурсы", detail: "Ленивая загрузка, аналитика и фоновые запросы продолжаются после первого кадра.", signal: "idle → lazy fetches", ms: 20, exchanges: 1 },
...
const russianSubstepPurpose: Record<string, string> = {
  ...
  "Корневой сервер и TLD": "Находит нужную доменную зону",
  ...
  "Приём сетевой картой": "Принимает и проверяет кадр на сервере",
  ...
  "Ленивая загрузка и фоновые ресурсы": "Продолжает некритическую загрузку",
...
const russianTechnicalDetails: Record<string, string> = {
  ...
  "Корневой сервер и TLD": "Корневой сервер возвращает ссылку на серверы зоны верхнего уровня, а TLD-сервер — на авторитативные серверы домена. Финального IP-адреса сайта в этих ответах ещё нет.",
  ...
  "Интерпретация ввода": "Адресная строка применяет эвристики к строке и её контексту: схеме, пробелам, истории и политике поисковой системы. Результатом становится либо URL навигации, либо URL поискового запроса, либо внутренняя команда браузера.",
  ...
  "Защита DDoS": "Edge агрегирует телеметрию L3–L7, применяет rate limiting, challenge, connection limits и фильтрацию известных аномалий. Цель — поглотить объём атаки до того, как origin-сервер исчерпает каналы или вычислительные ресурсы.",
  ...
  "Приём сетевой картой": "PHY восстанавливает сигнал, а MAC проверяет длину, адрес и FCS кадра. RSS может по хешу потока выбрать RX queue и процессор для дальнейшей обработки.",
  ...
  "Дерево доступности": "Нативная семантика HTML и ARIA преобразуются в роли, имена, состояния и отношения. Браузер инкрементально публикует дерево через accessibility API ОС для скринридеров и других вспомогательных технологий.",
  ...
  "Ленивая загрузка и фоновые ресурсы": "Viewport и scheduler запускают lazy images, prefetch, analytics и обновление Service Worker с подходящим приоритетом. Такая работа делит сеть, CPU и память с интерактивными задачами страницы.",
